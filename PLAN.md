...
- [x] **Liste des utilisateurs dans /admin** ✓ (2026-06-09) : endpoint `GET /admin/api/users` (monitor connecté à PostgreSQL en lecture), section "Utilisateurs" (avatar, email, plan, nb serveurs, date). Le monitor doit être connecté au réseau `sgrentmcserver_sgrent` + recevoir `DATABASE_URL`. `internal/monitor/users.go`.
